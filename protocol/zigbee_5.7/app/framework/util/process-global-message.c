...
#ifndef EMBER_AF_DISCOVER_ATTRIBUTES_RESPONSE_MAXIMUM_ATTRIBUTE_LIMIT
  #define EMBER_AF_DISCOVER_ATTRIBUTES_RESPONSE_MAXIMUM_ATTRIBUTE_LIMIT 5
#endif
  ...
  case ZCL_DISCOVER_ATTRIBUTES_EXTENDED_COMMAND_ID:
    {
      ...
      // BUGZID: EMAPPFWKV2-828
      // If we try to return more than 5 attributes at a time we run the
      // risk of a) building a tx buffer that is too large and failing to respond
      // or b) responding that we have completed discovery with a truncated buffer. 
      // Therefore we are throttling attribute discovery to !>5 attributes in a response.
      if (numberAttributes > EMBER_AF_DISCOVER_ATTRIBUTES_RESPONSE_MAXIMUM_ATTRIBUTE_LIMIT)
		  numberAttributes = EMBER_AF_DISCOVER_ATTRIBUTES_RESPONSE_MAXIMUM_ATTRIBUTE_LIMIT;
