...
uint8_t emberAfApsRetryOverride = EMBER_AF_RETRY_OVERRIDE_NONE;
...
PGM uint8_t emberAfDaysInMonth[] =
  { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
#endif

static PGM uint8_t emberAfAnalogDiscreteThresholds[] = {
  ...
  if (cmd.apsFrame->destinationEndpoint == EMBER_BROADCAST_ENDPOINT) {
    ...
    for (i = 0; i < emberAfEndpointCount(); i++) {
      ...
      if (!emberAfEndpointIndexIsEnabled(i)
          || !emberAfContainsCluster(endpoint, cmd.apsFrame->clusterId)) {
        continue;
      }
