...
// *****************************************************************************
  ...
  // In order to cast "const uint8_t*" to "const EmberEUI64" 
  // we must first cast away the const of the uint8_t* type and then cast the
  // data to a 'const EmberEUI64'.
  #define INT8U_TO_EUI64_CAST(x) ((const EmberEUI64)((uint8_t*)x)) 
#else
