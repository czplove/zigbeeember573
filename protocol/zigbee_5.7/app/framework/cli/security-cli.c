 ...
 *        - pan id - uint16_t. The 2 byte pan id of the partner with whom to start
 *          cbke.
...
#if !defined(XAP2B) && !defined(EMBER_AF_HAS_SECURITY_PROFILE_NONE)
...
void optionSecurityAllowTrustCenterRejoin(void)
{
  bool allow = (bool)emberUnsignedCommandArgument(0);
  emberAfCorePrintln("Trust Center %p allow trust center rejoins.",
                     ((emAfAllowTrustCenterRejoins = allow)
                      ? "WILL"
                      : "WILL NOT"));

  // Reinitialize the trust center security policy.
