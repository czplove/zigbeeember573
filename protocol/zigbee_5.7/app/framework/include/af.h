...
// Master array of all network types.
extern PGM EmAfNetworkType emAfNetworkTypes[];

// The current network type.
extern PGM EmAfNetworkType *emAfCurrentNetworkType;

// Master array of all ZigBee PRO networks.
extern PGM EmAfZigbeeProNetwork emAfZigbeeProNetworks[];

// The current ZigBee PRO network or NULL.
extern PGM EmAfZigbeeProNetwork *emAfCurrentZigbeeProNetwork;
 ...
 */
bool emberAfIsTypeSigned(uint8_t dataType);
 ...
 */
uint8_t emberAfGetDate(uint8_t* message, uint16_t currentIndex, uint16_t msgLen, EmberAfDate *dest);
...
/**
 * @brief Macro for consistency that copies an uint8_t from variable into buffer.
 */
 ...
 */
bool emberAfIsThisDataTypeAStringType(uint8_t dataType);
 ...
 */
void emberAfPrint(uint16_t functionality, PGM_P formatString, ...);
 ...
 * @param formatString: formatString for varargs
 */
void emberAfPrintln(uint16_t functionality, PGM_P formatString, ...);

/**
 * @brief buffer flush
 */
void emberAfFlush(uint16_t functionality);
...
/**
 * @brief Sends broadcast witdh alias with attached message sent callback.
 */
