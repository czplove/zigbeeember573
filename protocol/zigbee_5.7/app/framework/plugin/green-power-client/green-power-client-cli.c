...
void emberAfPluginGreenPowerClientSetProxyEntry(void)
{
  ...
  entry.sinkList[0].unicast.sinkNodeId = sinkNodeID;
  entry.sinkList[0].unicast.sinkEUI[0] = 0x84;
  entry.sinkList[0].unicast.sinkEUI[1] = 0x40;
  entry.sinkList[0].unicast.sinkEUI[2] = 0x18;
  entry.sinkList[0].unicast.sinkEUI[3] = 0x00;
  entry.sinkList[0].unicast.sinkEUI[4] = 0x00;
  entry.sinkList[0].unicast.sinkEUI[5] = 0x00;
  entry.sinkList[0].unicast.sinkEUI[6] = 0x00;
  entry.sinkList[0].unicast.sinkEUI[7] = 0x00;
    ...
    if (emGpProxyTableEntryInUse(i)) {
      ...
      for (j = 0; j < 2; j++) {
        if (emGpProxyTable[i].sinkList[j].type == EMBER_GP_SINK_TYPE_UNUSED) {
          ...
          emberAfCorePrint("GC %2x",emGpProxyTable[i].sinkList[j].groupcast.groupID);
        } else if (emGpProxyTable[i].sinkList[j].type == EMBER_GP_SINK_TYPE_LW_UNICAST) {
          emberAfCorePrint("LU:");
          emberAfPrintBigEndianEui64(emGpProxyTable[i].sinkList[j].unicast.sinkEUI);
        }
        emberAfCorePrint(" ");
      }
      emberAfCorePrint(" ");
