  ...
  if (entry->options & EMBER_AF_GP_PROXY_TABLE_ENTRY_OPTIONS_LIGHTWEIGHT_UNICAST_GPS){
    ...
    for (index = 0; index<GP_SINK_LIST_ENTRIES; index++){
      ...
      if (sinkEntry->type == EMBER_GP_SINK_TYPE_LW_UNICAST){
        emberAfPutBlockInResp(sinkEntry->unicast.sinkEUI, EUI64_SIZE);
        emberAfPutInt16uInResp(sinkEntry->unicast.sinkNodeId);
        *len +=  EUI64_SIZE + 2;
      } else {
        continue;
      }
  ...
  if (entry->options & EMBER_AF_GP_PROXY_TABLE_ENTRY_OPTIONS_COMMISIONED_GROUP_GPS){
    ...
    for (index = 0; index<GP_SINK_LIST_ENTRIES; index++){
      ...
      if ((sinkEntry->type == EMBER_GP_SINK_TYPE_D_GROUPCAST)
        ...
        emberAfPutInt16uInResp(sinkEntry->groupcast.groupID);
        emberAfPutInt16uInResp(sinkEntry->groupcast.alias);
        *len +=  4;
      } else {
        continue;
      }
  ...
  for (i = 0; i < GP_SINK_LIST_ENTRIES; i++) {
    if (sinkList[i].type == EMBER_GP_SINK_TYPE_FULL_UNICAST
      ...
      if(rxAfterTx)
        ...
        emSetAddDelay(GP_DMIN_U);
      UNUSED EmberStatus retval = emberAfSendCommandMulticastWithAlias(sinkList[i].groupcast.groupID,
                                                                emGpdAlias(addr),
                                                                sequenceNumber);
      emSetAddDelay(0);
                                      ...
                                      uint8_t randomSeqNum)
{
  ...
  expirationTimeList[entryIndex] = curTime + 2 * TICKS_PER_MIN;
