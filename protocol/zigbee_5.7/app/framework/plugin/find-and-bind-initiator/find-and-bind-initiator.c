...
static void processClusterList(uint8_t clusterCount,
                               const EmberAfClusterId *clusters,
                               EmberStatus *status,
                               bool *sendAddGroupToTarget);
...
EmberStatus emberAfPluginFindAndBindInitiatorStart(uint8_t endpoint)
{
  ...
  if (status == EMBER_SUCCESS) {
    emberEventControlSetDelayMS(checkTargetResponsesEventControl,
                                EMBER_AF_PLUGIN_FIND_AND_BIND_TARGET_RESPONSES_DELAY_MS);
    state = FIND_TARGETS;
  } else {
...
void emberAfPluginFindAndBindInitiatorCheckTargetResponsesEventHandler(void)
{
  ...
  if (status != EMBER_SUCCESS || finished) cleanupAndStop(status);
}
...
static void handleSimpleDescriptorResponse(const EmberAfServiceDiscoveryResult *result)
{
  ...
  bool sendAddGroupToTarget = true;
  ...
  processClusterList(clusterList->inClusterCount,
                     clusterList->inClusterList,
                     &status,
                     &sendAddGroupToTarget);
  processClusterList(clusterList->outClusterCount,
                               ...
                               bool *sendAddGroupToTarget)
{
  ...
  for (i = 0; (*status != EMBER_TABLE_FULL) && (i < clusterCount); i ++) {
    ...
    if (emberAfContainsClient(initiatorEndpoint, clusters[i])
        && emberAfPluginFindAndBindInitiatorBindTargetCallback(currentTargetInfoNodeId,
                                                               &bindingEntry,
                                                               groupName)) {
      *status = writeSimpleDescriptorResponse(&bindingEntry,
                                              groupName,
                                              sendAddGroupToTarget);
                                                 ...
                                                 bool *sendAddGroupToTarget)
{
  ...
  if (goodIndex < bindingTableSize) {
    ...
    if (newEntry->type == EMBER_MULTICAST_BINDING && *sendAddGroupToTarget) {
      uint16_t groupId
