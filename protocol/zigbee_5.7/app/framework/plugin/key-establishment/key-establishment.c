...
PGM uint8_t emAfKeyEstablishMessageToDataSize[] = {
...
static void checkSupportedCurve(bool initiateAsServer);
                                     ...
                                     uint8_t *data2)
{
...

  {
    ...
    uint32_t timeMs = (uint32_t)(eventTimeoutsSec[newEvent]) << 10;
                                            ...
                                            uint8_t endpoint)
{
  ...
  if (isCbkeKeyEstablishmentSuiteValid() && lastEvent == NO_KEY_ESTABLISHMENT_EVENT) {
    ...
    if(!partner.isIntraPan){
      ...
      if (emAfAvailableCbkeSuite & EMBER_AF_CBKE_KEY_ESTABLISHMENT_SUITE_283K1) {
        suite = EMBER_AF_CBKE_KEY_ESTABLISHMENT_SUITE_283K1;
      }
      else if (emAfAvailableCbkeSuite & EMBER_AF_CBKE_KEY_ESTABLISHMENT_SUITE_163K1) {
