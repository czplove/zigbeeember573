...
static bool emberAfIsZllDecisionId(EmberZllPolicy decisionId);
#ifdef EMBER_AF_PLUGIN_ZLL_COMMISSIONING_LINK_INITIATOR
...
void emberAfPluginZllCommissioningStackStatusCallback(EmberStatus status)
{
  ...
  if (formingNetwork()) {
    if (status == EMBER_NETWORK_UP) {
      // Make ourselves not factory new, and reset our address assignment
      // since we are starting a new network. We are not using the first node ID,
      // so that one is up for grabs.
      ...
      token.freeNodeIdMin = EMBER_ZLL_MIN_NODE_ID;
...
static void identifyRequestHandler(const EmberEUI64 source,
                                   ...
                                   uint16_t identifyDurationS)
{
  if((!emberAfIsZllDecisionId(EMBER_ZLL_POLICY_NO_TOUCHLINK_FOR_NFN)) ||
      (amFactoryNew())) {
