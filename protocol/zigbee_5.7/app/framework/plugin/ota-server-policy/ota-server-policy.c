...
static uint16_t imageBlockRequestMinRequestPeriodSec = 0;
...
void emAfOtaServerPolicyPrint(void)
{
  ...
  otaPrintln("Image Block Request Min Period: %d seconds",
             imageBlockRequestMinRequestPeriodSec);
  emberAfCoreFlush();
...
uint8_t emAfOtaServerImageBlockRequestCallback(EmberAfImageBlockRequestCallbackStruct* data)
{
  if (SERVER_AND_CLIENT_SUPPORT_MIN_BLOCK_REQUEST
      == (data->bitmask & SERVER_AND_CLIENT_SUPPORT_MIN_BLOCK_REQUEST)) {
    if (data->minBlockRequestPeriod != imageBlockRequestMinRequestPeriodSec) {
      data->minBlockRequestPeriod = imageBlockRequestMinRequestPeriodSec;
...
void emAfOtaServerPolicySetMinBlockRequestPeriod(uint16_t minBlockRequestPeriodSec)
{
  imageBlockRequestMinRequestPeriodSec = minBlockRequestPeriodSec;
}
