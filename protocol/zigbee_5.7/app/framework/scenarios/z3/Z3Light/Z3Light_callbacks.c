...
static void startFindingAndBindingTargetProcedure(void)
{
  EmberStatus status = emberAfPluginFindAndBindTargetStart(LIGHT_ENDPOINT);
  emberAfCorePrintln("Find and bind target start: 0x%X", status);
}
...
bool emberAfStackStatusCallback(EmberStatus status)
{
  ...
  if (status == EMBER_NETWORK_DOWN) {
    ...
    emberAfCorePrintln("%p network %p: 0x%X", "Open", "", status);
  }
                                                  ...
                                                  uint8_t finalState)
{
  ...
  if (status == EMBER_SUCCESS) {
    startFindingAndBindingTargetProcedure();
    emberEventControlSetActive(commissioningLedEventControl);
  } else {
