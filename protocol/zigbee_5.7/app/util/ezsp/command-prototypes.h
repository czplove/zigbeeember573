...
EmberStatus ezspRf4ceSetPairingTableEntry(
      // The index of the pairing table entry to be set.
      uint8_t pairingIndex,
      // The EmberRf4cePairingTableEntry struct to be copied into the pairing
      // table at the passed index.
...
EmberStatus ezspRf4ceGetPairingTableEntry(
      // The index of the requested pairing table entry.
      uint8_t pairingIndex,
      // Return: An EmberRf4cePairingTableEntry struct containing a copy of the
      // requested pairing entry.
      EmberRf4cePairingTableEntry *entry);

// Deletes the pairing table entry corresponding to the passed index.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceDeletePairingTableEntry(
      // The index of the pairing table entry to be deleted.
      uint8_t pairingIndex);

// Retrieves the pairing table entry stored at the passed index.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceKeyUpdate(
      // The index of the pairing table entry to be updated.
      uint8_t pairingIndex,
      // A pointer to an EmberKeyData struct containing the new key.
      EmberKeyData *key);

// Sends a message as per the ZigBee RF4CE specification.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceSend(
      // The index of the entry in the pairing table to be used to transmit the
      // packet. This parameter is ignored if broadcast bit is set in the
      // txOptions bitmask.
      uint8_t pairingIndex,
      // The profile ID to be included in the RF4CE network header of the
      // outgoing RF4CE network DATA frame.
      uint8_t profileId,
      // The vendor ID to be included in the RF4CE network header of the
      // outgoing RF4CE network DATA frame. This field is meaningful only if the
      // EMBER_RF4CE_TX_OPTIONS_VENDOR_SPECIFIC_BIT is set in the txOptions
      // bitmask.
      uint16_t vendorId,
      // 7-bit transmission options bitmask as per ZigBee RF4CE specification.
      EmberRf4ceTxOption txOptions,
      // A value chosen by the application. This value will be passed in the
      // corresponding ::ezspR4fceMessageSentHandler() response.
      uint8_t messageTag,
...
// The node starts the network operations.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceStart(
      // The node capabilities.
      EmberRf4ceNodeCapabilities capabilities,
      // The node vendor information.
      EmberRf4ceVendorInfo *vendorInfo,
      // The radio power the node should use in transmission.
      int8_t power);

// The node terminates the network operations.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceStop(void);

// The node performs a discovery of ZigBee RF4CE nodes that match the
// requirements specified in the passed parameters.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceDiscovery(
      // The PAN ID of the destination device for the discovery. This value can
      // be set to EMBER_RF4CE_BROADCAST_PAN_ID to indicate a wildcard.
      EmberPanId panId,
      // The network address of the destination device for the discovery. This
      // value can be set to EMBER_RF4CE_BROADCAST_ADDRESS to indicate a
      // wildcard.
      EmberNodeId nodeId,
      // The device type to discover. This value can be set to 0xFF to indicate
      // a wildcard.
      uint8_t searchDevType,
      // The time (in milliseconds) to wait for discovery responses to be sent
      // back from potential target nodes on each channel.
      uint16_t discDuration,
      // The maximum number of discovery trials. A discovery trial is defined as
      // the transmission of a discovery request command frame on all available
      // channels.
      uint8_t maxDiscRepetitions,
      // The length of the discovery profile ID list. The stack supports up to 7
      // profile ID entries.
      uint8_t discProfileIdListLength,
      // The list of profile IDs against which profile IDs contained in received
      // discovery response command frames will be matched for acceptance.
...
// The node automatically handles the receipt of discovery request command
// frames.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceEnableAutoDiscoveryResponse(
      // The maximum duration, in milliseconds, while the node will be in auto
...
// The node initiates the Rf4CE pairing process according to the specified
// parameters.
// Return: An EmberStatus value indicating success or the reason for failure.
...
// The node attempts to remove the specified pairing link.
// Return: An EmberStatus value indicating success or the reason for failure.
...
// The node enables or disables RF4CE power saving mode according to the passed
// parameters.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceSetPowerSavingParameters(
      // The duty cycle of a device in milliseconds.
      uint32_t dutyCycle,
      // The active period of a device in milliseconds.
      uint32_t activePeriod);

// Every target node periodically performs an energy scan of the base channel to
// detect whether the channel becomes compromised. In particular if
...
EmberStatus ezspRf4ceSetFrequencyAgilityParameters(
      // Defines the size of the RSSI reads window, that is, the number of the
      // most recent RSSI reads that are taken into consideration to decide
      // whether a channel switch is required or not. Valid values for this
      // parameter fall in the interval [1,32]. Setting this parameter to 0
      // disables frequency agility at the target. This parameter is set by
      // default to 8.
      uint8_t rssiWindowSize,
      // Defines the number of RSSI reads above the RSSI threshold that will
      // trigger a channel switch. Valid values for this parameter fall in the
      // interval [1,rssiWindowSize]. This parameter is set by default to 15.
      uint8_t channelChangeReads,
      // Defines the RSSI threshold. A RSSI value resulting from the periodic
      // scan that is above this threshold is considered a 'channel congested'
      // read. If the stack detects multiple channel congested reads, it will
      // eventually move the base channel to the next RF4CE channel. This
      // parameter is set by default to the CCA threshold.
      int8_t rssiThreshold,
      // The interval length (in seconds) between two consecutive RSSI reads.
      // This parameter is set by default to 10 seconds.
      uint16_t readInterval,
      // Sets the exponent of the number of scan periods of the RSSI read
      // process, where a scan period is 960 symbols, and a symbol is 16
      // microseconds. The scan will occur for ((2^duration) + 1) scan periods.
      // The value of duration must be less than 15. The time corresponding to
      // the first few values are as follows: 0 = 31 msec, 1 = 46 msec, 2 = 77
      // msec, 3 = 138 msec, 4 = 261 msec, 5 = 507 msec, 6 = 998 msec. This
      // parameter is set by default to 0.
      uint8_t readDuration);

// Sets the application information of the node.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceSetApplicationInfo(
      // A pointer to an ::EmberRf4ceApplicationInfo containing the application
      // information to be set at the node.
      EmberRf4ceApplicationInfo *appInfo);

// Retrieves the application information of the node.
// Return: An EmberStatus value indicating success or the reason for failure.
EmberStatus ezspRf4ceGetApplicationInfo(
      // Return: A pointer to an ::EmberRf4ceApplicationInfo where the stack
      // will copy the application information of the node.
      EmberRf4ceApplicationInfo *appInfo);

// Retrieves the application information of the node.
// Return: The maximum allowed payload in bytes according to the passed pairing
// index and TX options.
uint8_t ezspRf4ceGetMaxPayload(
      // The index of the pairing table entry a packet shall be sent to.
      uint8_t pairingIndex,
      // The TX options bitmask.
      EmberRf4ceTxOption txOptions);

// Returns the current network parameters for the RF4CE network, if there is
// one. If the current network is not RF4CE, this function will return
// ::EZSP_ERROR_INVALID_CALL.
// Return: An EmberStatus value indicating success or the reason for failure.
...
void ezspGpProxyTableProcessGpPairing(
