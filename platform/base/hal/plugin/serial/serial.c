                                          ...
                                          uint8_t stopBits)
{
  ...
  {
    initdata.uartdrvinit.baudRate=rate;
    initdata.uartdrvinit.parity=(USART_Parity_TypeDef)parity;
    if (stopBits==1)
    {
      initdata.uartdrvinit.stopBits=usartStopbits1;
    }
    else if (stopBits==2)
    {
      initdata.uartdrvinit.stopBits=usartStopbits2;
    }
...
EmberStatus emberSerialGuaranteedPrintf(uint8_t port, PGM_P formatString, ...)
{
  ...
  if(!emPrintfInternal(COM_ForceWriteData, (COM_Port_t) port, formatString, ap))
