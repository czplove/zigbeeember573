...
EmberGpProxyTableEntry *emGpProxyTable = (EmberGpProxyTableEntry *) &emAvailableMemory[END_emRf4cePendingOutgoingPacketTable];
uint8_t emGpProxyTableSize = EMBER_GP_PROXY_TABLE_SIZE;
#define END_emGpProxyTable         \
  (END_emRf4cePendingOutgoingPacketTable+ align(EMBER_GP_PROXY_TABLE_SIZE * sizeof(EmberGpProxyTableEntry)))
...
uint16_t *emBindingRemoteNode = (uint16_t *) &emAvailableMemory[END_emGpSinkTable];
uint8_t emberBindingTableSize = EMBER_BINDING_TABLE_SIZE;
#define END_emBindingRemoteNode        \
 (END_emGpSinkTable + align(EMBER_BINDING_TABLE_SIZE * sizeof(uint16_t)))
