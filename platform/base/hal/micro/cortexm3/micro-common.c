...
void halGpioSetConfig(uint32_t gpio, HalGpioCfg_t config)
{
  ...
  uint32_t address = ((gpio & 0x4) ? GPIO_PxCFGH_BASE : GPIO_PxCFGL_BASE) + port * GPIO_Px_OFFSET;
...
HalGpioCfg_t halGpioGetConfig(uint32_t gpio)
{
  ...
  uint32_t address = ((gpio & 0x4) ? GPIO_PxCFGH_BASE : GPIO_PxCFGL_BASE) + port * GPIO_Px_OFFSET;
  uint32_t shift =  (gpio & 0x3) * 4;
  uint32_t saved = *((volatile uint32_t *)address);
...
{
  uint32_t port = gpio / 8;
  uint32_t address = GPIO_PxCLR_BASE + port * GPIO_Px_OFFSET;
  uint32_t pin = gpio & 0x7;
  *((volatile uint32_t *) address) = BIT(pin);
}

void halGpioSet(uint32_t gpio)
{
  uint32_t port = gpio / 8;
  uint32_t address = GPIO_PxSET_BASE + port * GPIO_Px_OFFSET;
  uint32_t pin = gpio & 0x7;
  *((volatile uint32_t *) address) = BIT(pin);
}
...
//VREG register.  The 1V8_TEST bit must be set directly control 1V8_EN.
...
void halCommonEnableVreg1v8(void)
{
  ATOMIC(
    ...
    if (++halCommonVreg1v8EnableCount == 1) {
      VREG = ( ( VREG & (~(VREG_VREG_1V8_EN_MASK | VREG_VREG_1V8_TEST_MASK)) ) |
             (1 << VREG_VREG_1V8_EN_BIT)                                       |
             (0 << VREG_VREG_1V8_TEST_BIT)                                     );
    }
  )
}
#else
