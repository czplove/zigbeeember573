 ...
 * PB6 * BUTTON0/IRQB MCM_PB6           IN_PUD^   IN_PUD^W (non-NCP apps)
 *    `- nRHO         nRHO              IN_PUD^   IN_PUD^W RHO_GPIO+RADIO_HOLDOFF
 ...
 *     * FEM_CPS      FEM_CPS           OUT^      OUTv     (NCP apps)
 * PC2 * SWO          MCM_PC2/JTDO      OUT_ALT^  OUT^
 * PC3 * JTDI         MCM_PC3/JTDI      INv       IN_PUDv
 * PC4 * SWDIO        MCM_PC4/SWDIO     INv       IN_PUDv
 * PC5 * LED2         -n/c-             OUT^      OUT^
 *    `- TX_ACTIVE    FEM_CTX           OUT_ALTv  OUTv     ENABLE_ALT_FUNCTION_TX_ACTIVE
 * PC6 * BUTTON1      MCM_PC6           IN_PUD^   IN_PUD^W
 *    `- nTX_ACTIVE   FTM_CRX           OUT_ALTv  OUTv     ENABLE_ALT_FUNCTION_NTX_ACTIVE
...
#define RHO_GPIO              PORTB_PIN(6) // Ensure RHO configured for NCP
#else//!(defined(EZSP_SPI) || defined(EZSP_ASH))
//#define RHO_GPIO              PORTB_PIN(6)
...
/**
 * @brief Deal with NCP variations
 */
#if     (defined(EZSP_SPI) || defined(EZSP_ASH))
  #define PWRDN_OUT_FEM_CPS     0 // For FEM_CPS
#else//!(defined(EZSP_SPI) || defined(EZSP_ASH))
  #define PWRDN_OUT_FEM_CPS     1 // For LED
#endif//(defined(EZSP_SPI) || defined(EZSP_ASH))
