...
#include "stack/framework/packet-buffer.h"
...
static void softwareUartTxByte(uint8_t byte)
{
  uint8_t i;
  ...
  uint16_t fullBitTime = (uint16_t)(((FULL_BIT_TIME_SCLK
                               * (halMcuClockHz() / 250000UL)) + 48) / 96);
...
void halInternalStartUartTx(uint8_t port)
{
    ...
    if (EM_SER1_PORT_EN(port)) {
      ...
      CONFIG_SOFT_UART_TX_BIT();
      while (q->used > 0) {
        uint8_t byte = FIFO_DEQUEUE(q, emSerialTxQueueWraps[1]);
...
#ifdef SOFTUART
//this requires use of the SysTick counter and will destroy interrupt latency!
static uint8_t softwareUartRxByte(void)
{
  uint8_t i;
  uint8_t bit;
  uint8_t byte = 0;
  ...
  // quad arithmetic dividing it by 250000.)
  uint16_t fullBitTime  = (uint16_t)((( FULL_BIT_TIME_SCLK
                                      * (halMcuClockHz() / 250000)) + 48) / 96);
  uint16_t startBitTime = (uint16_t)(((START_BIT_TIME_SCLK
                                      * (halMcuClockHz() / 250000)) + 48) / 96);
  ATOMIC(
    INTERRUPTS_ON();
