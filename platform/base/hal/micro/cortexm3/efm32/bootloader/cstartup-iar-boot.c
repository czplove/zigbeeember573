 ...
 * the main() found in C code.  cstartup-iar-boot-asm instantiates the actual 
 * reset vector since hand crafted assembly code is required to get from the 
 * bootloader to the application cstartup when exiting deep sleep.
 *******************************************************************************
 * @section License
...
void bootloaderEntryPoint(bool isForced)
{
  ...
  // Some emlib and emdrv functions use INT_Enable and INT_Disable in em_int to
  // implement critical sections. While em_int does keep track of nesting with a
  // counter, it does not save the previous interrupt state. When the last 
  // INT_Enable is called and the counter decrements to 0, interrupts will be
  // enabled unconditionally. Adding an early call to INT_Disable increments
  // the counter and should prevent this (assuming all calls are matched).
  // This must be done after memory initialization.
  INT_Disable();
...
VAR_AT_SEGMENT(NO_STRIPPING const HalBootloaderAddressTableType halBootloaderAddressTable, __BAT__) = {
  ...
  SOFTWARE_VERSION, //uint16_t bootloaderVersion;  
  &halAppAddressTable,
