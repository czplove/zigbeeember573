...
#endif
#include "hal/hal.h"
#include "hal/micro/power-meter.h"
...
#define CURRENT_GAIN_DEFAULT                0x00340000
...
static void cs5463WriteReg(uint8_t page, uint8_t cs5463Register, uint8_t * writeBuffer);
static void cs5463ReadReg(uint8_t page, uint8_t cs5463Register, uint8_t * readBuffer);
static void cs5463WriteCommand(uint8_t command);
static uint32_t cs5463GetStatus(void);
static int16_t cs5463GetTemperature(bool isCalibrate);
static int32_t cs5463GetACVoltageOffset(void);
static int32_t cs5463GetACCurrentOffset(void);
static int32_t cs5463GetDCVoltageOffset(void);
static int32_t cs5463GetDCCurrentOffset(void);
static void cs5463Calibrate(uint8_t option, 
                            HalPowerMeterCalibrationData * pCalibrationData);
static int32_t cs5463CalTemp(int16_t temperatureRef);
...
static uint16_t exceptionCallbackCounter; 
static uint16_t exceptionTimeCounter;
#if defined(CORTEXM3_EFR32)
...
void halPowerMeterInit(void)
{
  ...
  *((volatile uint32_t *)(GPIO_PxSET_BASE + 
  ...
  writeBuffer[1] = BYTE_2(TEMPERATURE_OFFSET_DEFAULT);
  writeBuffer[2] = BYTE_1(TEMPERATURE_OFFSET_DEFAULT);
  writeBuffer[3] = BYTE_0(TEMPERATURE_OFFSET_DEFAULT);
  cs5463WriteReg(PAGE_1, CS5463_TEMP_OFFSET, writeBuffer);

  writeBuffer[1] = BYTE_2(VOLTAGE_AC_OFFSET_DEFAULT);
  writeBuffer[2] = BYTE_1(VOLTAGE_AC_OFFSET_DEFAULT);
  writeBuffer[3] = BYTE_0(VOLTAGE_AC_OFFSET_DEFAULT);
  cs5463WriteReg(PAGE_0, CS5463_VACOFF_REG, writeBuffer);
  
  writeBuffer[1] = BYTE_2(CURRENT_AC_OFFSET_DEFAULT);
  writeBuffer[2] = BYTE_1(CURRENT_AC_OFFSET_DEFAULT);
  writeBuffer[3] = BYTE_0(CURRENT_AC_OFFSET_DEFAULT);
  cs5463WriteReg(PAGE_0, CS5463_IACOFF_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(CURRENT_DC_OFFSET_DEFAULT);
  writeBuffer[2] = BYTE_1(CURRENT_DC_OFFSET_DEFAULT);
  writeBuffer[3] = BYTE_0(CURRENT_DC_OFFSET_DEFAULT);
  cs5463WriteReg(PAGE_0, CS5463_CURRENTDCOFFSET_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(VOLTAGE_DC_OFFSET_DEFAULT);
  writeBuffer[2] = BYTE_1(VOLTAGE_DC_OFFSET_DEFAULT);
  writeBuffer[3] = BYTE_0(VOLTAGE_DC_OFFSET_DEFAULT);
  cs5463WriteReg(PAGE_0, CS5463_VOLTAGEDCOFFSET_REG, writeBuffer);
  
  writeBuffer[1] = BYTE_2(DEFAULT_CYCLE_COUNT);
  writeBuffer[2] = BYTE_1(DEFAULT_CYCLE_COUNT);
  writeBuffer[3] = BYTE_0(DEFAULT_CYCLE_COUNT);
  cs5463WriteReg(PAGE_0, CS5463_CYCLECOUNT_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(CURRENT_GAIN_DEFAULT);
  writeBuffer[2] = BYTE_1(CURRENT_GAIN_DEFAULT);
  writeBuffer[3] = BYTE_0(CURRENT_GAIN_DEFAULT);
  cs5463WriteReg(PAGE_0, CS5463_CURRENTGAIN_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(VOLTAGE_GAIN_DEFAULT);
  writeBuffer[2] = BYTE_1(VOLTAGE_GAIN_DEFAULT);
  writeBuffer[3] = BYTE_0(VOLTAGE_GAIN_DEFAULT);
  cs5463WriteReg(PAGE_0, CS5463_VOLTAGEGAIN_REG, writeBuffer);

  config=((PC) | (IGAIN) | (EWA) | (IMODE ) | (IINV) | (ICPU) | (CDIV));
  writeBuffer[1] = BYTE_2(config);
  writeBuffer[2] = BYTE_1(config);
  writeBuffer[3] = BYTE_0(config);

  statusOverHeat = CS5463_OVER_HEAT_TO_NORMAL;
  statusOverCurrent = CS5463_OVER_CURRENT_TO_NORMAL;
  exceptionTimeCounter = 0;
  exceptionCallbackCounter = 0;

  cs5463WriteReg(PAGE_0, CS5463_CONFIG_REG, writeBuffer);
...
static uint32_t cs5463GetStatus(void)
{
  ...
  status = INT8U_TO_INT32U(0, readBuffer[1], readBuffer[2], readBuffer[3]);
  return status;
}
...
static int16_t cs5463GetTemperature(bool isCalibrate)
{
  ...
  cs5463ReadReg(PAGE_0, CS5463_TEMPERATURE_REG, readBuffer);
  temperature = (int32_t)INT8U_TO_INT32U(0, 
                                        readBuffer[1],
                                        readBuffer[2],
                                        readBuffer[3]);
  ...
  if (isCalibrate) {
    ...
    if ((statusOverHeat == CS5463_NORMAL_TO_OVER_HEAT) && 
      ...
      exceptionTimeCounter = exceptionTimeCounter - 1;
      if (exceptionTimeCounter == 0) {
        statusOverHeat = CS5463_OVER_HEAT_TO_NORMAL;
        halPowerMeterCs5463OverHeatStatusChangeCallback(statusOverHeat);
      } 
    } else if ((statusOverHeat == CS5463_OVER_HEAT_TO_NORMAL) && 
               (temperature > CS5463_OVER_HEAT_THRESHOLD)) {
      statusOverHeat = CS5463_NORMAL_TO_OVER_HEAT;
      exceptionTimeCounter = CS5463_OVER_HEAT_TIME_LIMIT;
      exceptionCallbackCounter = CS5463_OVER_HEAT_TIME_LIMIT;
      halPowerMeterCs5463OverHeatStatusChangeCallback(statusOverHeat);
    } else if ((statusOverHeat == CS5463_NORMAL_TO_OVER_HEAT) && 
               (temperature > CS5463_OVER_HEAT_THRESHOLD)) {
      exceptionCallbackCounter = exceptionCallbackCounter - 1;
      if (exceptionCallbackCounter == 0) {
        exceptionCallbackCounter = CS5463_OVER_HEAT_TIME_LIMIT;
        halPowerMeterCs5463OverHeatStatusChangeCallback(statusOverHeat);
      } 
    }
...
static int32_t cs5463GetACVoltageOffset(void)
{
  return cs5463GetSignedInt32FromRegister(CS5463_VACOFF_REG);
}

static int32_t cs5463GetACCurrentOffset(void)
{
  return cs5463GetSignedInt32FromRegister(CS5463_IACOFF_REG);
}

static int32_t cs5463GetDCVoltageOffset(void)
{
  return cs5463GetSignedInt32FromRegister(CS5463_VOLTAGEDCOFFSET_REG);
}
                            ...
                            HalPowerMeterCalibrationData * pCalibrationData)
{
  ...
  *((volatile uint32_t *)(GPIO_PxSET_BASE + 
  ...
  // only difference in calibration mode is the gain must be 500mVp-p
  config = (PC | (0 << IGAIN_BIT) | EWA | IMODE | IINV | ICPU | CDIV);
  writeBuffer[1] = BYTE_2(config);
  writeBuffer[2] = BYTE_1(config);
  writeBuffer[3] = BYTE_0(config);
  cs5463WriteReg(PAGE_0, CS5463_CONFIG_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(CALIBRATE_CYCLE_COUNT);
  writeBuffer[2] = BYTE_1(CALIBRATE_CYCLE_COUNT);
  writeBuffer[3] = BYTE_0(CALIBRATE_CYCLE_COUNT);
  cs5463WriteReg(PAGE_0, CS5463_CYCLECOUNT_REG, writeBuffer);

  // clear the offset registers first
  writeBuffer[1] = 0x00;
  writeBuffer[2] = 0x00;
  writeBuffer[3] = 0x00;
  cs5463WriteReg(PAGE_0, CS5463_VACOFF_REG, writeBuffer);
  ...
  if (option == CS5463_CALIBRATE_AC) {
    ...
    cs5463WriteCommand(CS5463_CMD_CALIBRATION_CURRENTVOLTAGE_AC_OFFSET);
    while (dataReady == 0) {
      halCommonDelayMilliseconds(10);
      if (cs5463GetStatus() & CS5463_STATUS_DATAREADY) {
        dataReady = 1;
      }
    }
    pCalibrationData->acVoltageOffset = cs5463GetACVoltageOffset();
    pCalibrationData->acCurrentOffset = cs5463GetACCurrentOffset();
    
  }
  else if (option == CS5463_CALIBRATE_DC) {
    writeBuffer[1] = BYTE_2(CURRENT_UNIT_GAIN);
    writeBuffer[2] = BYTE_1(CURRENT_UNIT_GAIN);
    writeBuffer[3] = BYTE_0(CURRENT_UNIT_GAIN);
    cs5463WriteReg(PAGE_0, CS5463_CURRENTGAIN_REG, writeBuffer);

    writeBuffer[1] = BYTE_2(VOLTAGE_UNIT_GAIN);
    writeBuffer[2] = BYTE_1(VOLTAGE_UNIT_GAIN);
    writeBuffer[3] = BYTE_0(VOLTAGE_UNIT_GAIN);
    cs5463WriteReg(PAGE_0, CS5463_VOLTAGEGAIN_REG, writeBuffer);
  
    cs5463WriteCommand(CS5463_CMD_CALIBRATION_CURRENTVOLTAGE_DC_OFFSET);
    while (dataReady == 0){
      halCommonDelayMilliseconds(10);
      if (cs5463GetStatus() & CS5463_STATUS_DATAREADY ) {
        dataReady = 1;
    ...
    pCalibrationData->dcVoltageOffset = cs5463GetDCVoltageOffset();
    pCalibrationData->dcVoltageOffset = cs5463GetDCCurrentOffset();
  }
  halPowerMeterInit();
}
...
static int32_t cs5463CalTemp(int16_t temperatureRef)
{
  ...
  int16_t temperature;
  int16_t diff;
  int32_t calibrateData;

  cs5463ReadReg(PAGE_1, CS5463_TEMP_OFFSET, buffer);

  // check the msb of buffer[1] to see if it is a neative value
  // if it is negative, we need to convert the MSB of 32 bit calibrateData 
  // to negative as well. 
  calibrateData = (int32_t)INT8U_TO_INT32U(((buffer[1] & 0x80) ? 
                                                 (0xFF << 24) : 
                                                   0), 
                                            buffer[1], 
                                            buffer[2], 
                                            buffer[3]);
  temperature = cs5463GetTemperature(true);
  diff = temperatureRef - temperature;

  // if the difference is less than 0.5C, we don't do any calibration
  if ((diff < 50) && ( diff> -50)) {
    return calibrateData; 
  } else {
    // the difference in degreeC should be mulitply by the gain for adc value
    // calculation.
    calibrateData = TEMPERATURE_GAIN_DEFAULT_VALUE 
                    * ((int32_t)diff) 
                    + calibrateData;
    buffer[1] = BYTE_2(calibrateData);
    buffer[2] = BYTE_1(calibrateData);
    buffer[3] = BYTE_0(calibrateData);
    
    cs5463WriteReg(PAGE_1, CS5463_TEMP_OFFSET, buffer);
...
void halPowerMeterCalibrateAll(int16_t temperature, 
                               HalPowerMeterCalibrationData * pCalibrationData)
{
  cs5463Calibrate(CS5463_CALIBRATE_AC, pCalibrationData);
  cs5463Calibrate(CS5463_CALIBRATE_DC, pCalibrationData);
  pCalibrationData->temperatureOffset = cs5463CalTemp(temperature);
}
...
static uint32_t cs5463GetCrmsMilliA(void)
{
  ...
  if ((statusOverCurrent == CS5463_NORMAL_TO_OVER_CURRENT) && 
    ...
    if (exceptionTimeCounter == 0) {
      statusOverCurrent = CS5463_OVER_CURRENT_TO_NORMAL;
      halPowerMeterCs5463OverCurrentStatusChangeCallback(statusOverCurrent);
    } 
  } else if ((statusOverCurrent == CS5463_OVER_CURRENT_TO_NORMAL) && 
             (crms > CS5463_OVER_CURRENT_THRESHOLD)) {
    statusOverCurrent = CS5463_NORMAL_TO_OVER_CURRENT;
    exceptionTimeCounter = CS5463_OVER_CURRENT_TIME_LIMIT;
    exceptionCallbackCounter = CS5463_OVER_CURRENT_TIME_LIMIT;
    halPowerMeterCs5463OverCurrentStatusChangeCallback(statusOverCurrent);
...
static int32_t cs5463GetActivePowerMilliW(void)
{
  ...
  cs5463ReadReg(PAGE_0, CS5463_ACTIVEPOWER_REG, readBuffer);
  power = (int32_t) INT8U_TO_INT32U(0, 
                                    readBuffer[1],
                                    readBuffer[2],
                                    readBuffer[3]); 
  // active power is signed value, 
  // check the sign bit and make it a negative int32_t  
  if (power & BIT32(ADC_REGISTER_SIGN_BIT)) {
    power = CONVERT_NEG_24_BIT_TO_NEG_32_BIT(power);
  }
...
static int8_t cs5463GetPowerFactor(void)
{
  ...
  cs5463ReadReg(PAGE_0, CS5463_POWERFACTOR_REG, readBuffer);
  powerFactor = (int32_t) INT8U_TO_INT32U(0,
                                          readBuffer[1],
                                          readBuffer[2],
                                          readBuffer[3]); 
  ...
  if (powerFactor & BIT32(ADC_REGISTER_SIGN_BIT)) { 
    powerFactor = CONVERT_NEG_24_BIT_TO_NEG_32_BIT(powerFactor);
  }
...
void halPowerMeterWriteCalibrateData(HalPowerMeterCalibrationData * pCalibrateData)
{
  ...
  writeBuffer[1] = BYTE_2(pCalibrateData->temperatureOffset);
  writeBuffer[2] = BYTE_1(pCalibrateData->temperatureOffset);
  writeBuffer[3] = BYTE_0(pCalibrateData->temperatureOffset);
  cs5463WriteReg(PAGE_1, CS5463_TEMP_OFFSET, writeBuffer);
  
  writeBuffer[1] = BYTE_2(pCalibrateData->dcVoltageOffset);
  writeBuffer[2] = BYTE_1(pCalibrateData->dcVoltageOffset);
  writeBuffer[3] = BYTE_0(pCalibrateData->dcVoltageOffset);
  cs5463WriteReg(PAGE_0, CS5463_VOLTAGEDCOFFSET_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(pCalibrateData->dcCurrentOffset);
  writeBuffer[2] = BYTE_1(pCalibrateData->dcCurrentOffset);
  writeBuffer[3] = BYTE_0(pCalibrateData->dcCurrentOffset);
  cs5463WriteReg(PAGE_0, CS5463_CURRENTDCOFFSET_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(pCalibrateData->acVoltageOffset);
  writeBuffer[2] = BYTE_1(pCalibrateData->acVoltageOffset);
  writeBuffer[3] = BYTE_0(pCalibrateData->acVoltageOffset);
  cs5463WriteReg(PAGE_0, CS5463_VACOFF_REG, writeBuffer);

  writeBuffer[1] = BYTE_2(pCalibrateData->acCurrentOffset);
  writeBuffer[2] = BYTE_1(pCalibrateData->acCurrentOffset);
  writeBuffer[3] = BYTE_0(pCalibrateData->acCurrentOffset);
  cs5463WriteReg(PAGE_0, CS5463_IACOFF_REG, writeBuffer);
}
